...
`manifest.json` 仍然表示最新快照。`bindings.jsonl` 是 append-only 的 Git 风格历史，会记录 Codex 快照 bundle、同步批次、业务项目 branch、业务项目 `HEAD` commit，以及业务工作区当时是否 dirty。`bindings.idx.json` 是从 `bindings.jsonl` 派生出来的可重建查询缓存，用于加速 `log`、`show` 和 selector restore。
...
src/
  ...
  agents.js          # Codex state / JSONL 发现与扫描匹配
...
如果你的 Codex 会话不在默认路径，可以用环境变量覆盖。`AGENT_SYNC_CODEX_DIR` 可以指向 `.codex` 目录或 `.codex/sessions` 目录：
